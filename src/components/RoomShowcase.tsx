      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Choose from our collection of unique rooms and suites, each thoughtfully
            designed to provide the perfect Mykonos experience with unparalleled
            luxury and comfort.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {rooms.map((room) => (
            <RoomCard key={room.id} room={room} addToRefs={addToRefs} />
          ))}
        </div>
        ...
        <div className="text-center">
          <div className="inline-flex flex-col items-center space-y-6 p-12 bg-card/30 dark:bg-card/50 backdrop-blur-xl rounded-3xl border border-border/50">
            ...
            <p className="text-muted-foreground max-w-md">
              Book your perfect room today and discover the magic of Mykonos luxury
              hospitality.
            </p>
            <Button
              size="lg"
...
function RoomCard({
  ...
  // each card has its own slideIndex
  const [slideIndex, setSlideIndex] = useState<number>(0);

  return (
    <Card
      ref={addToRefs}
      className="opacity-0 group cursor-pointer bg-card/50 dark:bg-card/80 backdrop-blur-xl border border-border/50 transition-all duration-700 hover:scale-105 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10 overflow-hidden"
    >
      <CardContent className="p-0">
        <div className="relative h-80 lg:h-96 overflow-hidden">
          <Carousel
            ...
            loop
            value={slideIndex}
            onValueChange={(val) => setSlideIndex(val)}
          >
            <CarouselContent>
          ...
          {/* ensure this overlay doesn't block clicks */}
